...
//
// Genera imagen PNG 1200×630 compartible con la placa de un usuario.
// Pensada para Twitter card / OG meta tags / "Compartir mi placa".
//
// Diseño simplificado (sin clip-path ni conic-gradient — limitaciones
// de next/og ImageResponse). Mantiene la identidad visual: tier metálico,
// avatar circular con ring, nombre dramático, level prominente, badges.
//
// Runtime: nodejs (para poder usar fetch absolute al endpoint público
// /api/placa/[userId] que sí tiene todos los helpers).
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  ...
  // Fetch data del endpoint público
  let data: PlacaApiResponse | null = null
  try {
    const res = await fetch(`${origin}/api/placa/${userId}`, { cache: 'no-store' })
    if (res.ok) data = await res.json() as PlacaApiResponse
  } catch { /* fallback below */ }

  // Fallback si el user no existe o algo falla
  if (!data) {
    data = {
      displayName: 'Takero',
      handle: 'takero',
            ...
            <div style={{
              ...
              <div style={{
                ...
                {data.avatarUrl ? (
                  ...
                  <span style={{
                    fontSize: 40, fontWeight: 900, color: tier.primary,
                  }}>
                    {initials}
                  </span>
                )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{
              ...
              takasportsmedia.com/perfil/{userId.slice(0, 8)}
            </span>
          </div>
        </div>
      </div>
