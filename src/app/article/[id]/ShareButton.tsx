...
// El menú se abre SIEMPRE (antes en móvil iba directo a navigator.share). Hacía
// falta para que quepa "Compartir en historia", que es una acción distinta:
// comparte una imagen 1080×1920, no un enlace.
//
// Sobre el enlace de la historia: Meta retiró `contentURL` de su API de Sharing
// to Stories, así que NO se puede inyectar un link clicable en la historia de
// otra persona. Lo que sí funciona es dejarle la URL en el portapapeles para que
// la pegue en el sticker de enlace de Instagram — de ahí el copiado previo.
export default function ShareButton({ title, slug }: { title: string; slug?: string }) {
  ...
  const trackShare = (method: 'x' | 'whatsapp' | 'facebook' | 'copy' | 'story' | 'native') => {
    trackArticleShare({ title, method })
  }
  ...
  React.useEffect(() => {
    if (story === 'shared' || story === 'downloaded') {
  ...
  const itemStyle: React.CSSProperties = { color: '#D1D1E0' }
        ...
        >
          {slug && (
            <button
              onClick={shareStory}
              disabled={story === 'busy'}
              className="w-full flex items-center gap-3 px-4 py-3 text-xs hover:bg-white/5 transition-colors text-left disabled:opacity-70"
              style={{
                color: story === 'failed' ? '#F87171' : '#E9D5FF',
                background: 'linear-gradient(90deg, rgba(168,85,247,0.14), transparent)',
                borderBottom: '1px solid var(--border)',
                fontWeight: 600,
              }}
            >
              {story === 'busy' ? (
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="animate-spin flex-shrink-0">
                  <circle cx="7" cy="7" r="5.4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeDasharray="21 12" />
                </svg>
              ) : (
          ...
          {typeof navigator !== 'undefined' && 'share' in navigator && (
            <button
              ...
              style={itemStyle}
            >
              <svg width="14" height="14" viewBox="0 0 13 13" fill="none" className="flex-shrink-0">
                <circle cx="10.5" cy="2.5" r="1.5" stroke="currentColor" strokeWidth="1.3" />
                <circle cx="10.5" cy="10.5" r="1.5" stroke="currentColor" strokeWidth="1.3" />
                <circle cx="2.5" cy="6.5" r="1.5" stroke="currentColor" strokeWidth="1.3" />
                <path d="M4 5.8L9 3.2M4 7.2L9 9.8" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="flex-shrink-0"><path d="M18.244 2H21l-6.52 7.45L22 22h-6.793l-5.32-6.94L3.7 22H1l7.04-8.04L1 2h6.93l4.81 6.36L18.244 2zm-1.19 18.36h1.5L7.05 3.55H5.45L17.054 20.36z"/></svg>
            Compartir en X
          </a>
          <a
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="flex-shrink-0"><path d="M9.101 23.691v-7.98H6.627v-3.667h2.474v-1.58c0-4.085 1.848-5.978 5.858-5.978.401 0 .955.042 1.468.103a8.68 8.68 0 011.141.195v3.325a8.623 8.623 0 00-.653-.036 26.805 26.805 0 00-.733-.009c-.707 0-1.259.096-1.675.309a1.686 1.686 0 00-.679.622c-.258.42-.374.995-.374 1.752v1.297h3.919l-.386 2.103-.287 1.564h-3.246v8.245C19.396 23.238 24 18.179 24 12.044c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.628 3.874 10.35 9.101 11.647z"/></svg>
            Facebook
          </a>
          <button
            onClick={copyLink}
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="flex-shrink-0"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
