// Iconos monoline + mini previews compartidos entre /juegos y la home.

export function IconQuiniela({ size = 32 }: { size?: number }) {
  return (
...
export function IconCrackQuiz({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
...
export function IconMiOnce({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
...
export function IconSopaCracks({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
      <rect x="3" y="3" width="26" height="26" rx="3" stroke="currentColor" strokeWidth="1.6" />
...
export function IconTakaGrid({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
...
export function IconStrikerRush({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
...
export function PreviewCrackQuiz({ accent, accentDim }: { accent: string; accentDim: string }) {
  const opts = ['5', '6', '7 ✓', '8']
  return (
    <div className="w-full px-1 pb-1">
...
export function PreviewMiOnce({ accent }: { accent: string; accentDim?: string }) {
...
export function PreviewSopaCracks({ accent, accentDim }: { accent: string; accentDim: string }) {
...
// Preview Quiniela: papeleta con 1/X/2 estilo cupón
export function PreviewQuiniela({ accent, accentDim }: { accent: string; accentDim: string }) {
  const rows = [
    { home: 'RMA', away: 'BAR', pick: '1' },
    { home: 'ATM', away: 'SEV', pick: 'X' },
    { home: 'LIV', away: 'ARS', pick: '1' },
    { home: 'MUN', away: 'CHE', pick: '2' },
  ]
  return (
        ...
        <div className="space-y-1">
          {rows.map((r, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <span className="text-[9px] font-bold flex-1 truncate" style={{ color: 'rgba(255,255,255,0.55)' }}>{r.home} – {r.away}</span>
              <div className="flex gap-0.5">
                {(['1', 'X', '2'] as const).map(opt => {
                  const sel = opt === r.pick
                  return (
                    <div
                      key={opt}
                      className="w-4 h-4 rounded-sm flex items-center justify-center text-[8px] font-black"
                      style={{
                        background: sel ? `${accentDim}70` : 'rgba(255,255,255,0.04)',
                        color: sel ? '#fff' : 'rgba(255,255,255,0.25)',
                        border: sel ? `1px solid ${accent}80` : '1px solid rgba(255,255,255,0.06)',
                      }}
                    >
                      {opt}
                    </div>
